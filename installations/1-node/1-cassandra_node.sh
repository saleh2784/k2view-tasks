...
sed -i '11i\alias python='/usr/bin/python2.7'\' ~/.bash_profile source ./.bash_profile
...
sed -i 's@LOCAL_JMX=.*@LOCAL_JMX='n'@' $INSLATT_DIR/cassandra/conf/cassandra-env.sh
...
sed -i "s@-Dcom.sun.management.jmxremote.password.file=.*@-Dcom.sun.management.jmxremote.password.file=/opt/apps/cassandra/cassandra/conf/.jmxremote.password\"@" $INSLATT_DIR/cassandra/conf/cassandra-env.sh
{
    ...
    echo "controlRole R&D"
    echo "cassandra cassandra"
    echo "k2view Q1w2e3r4t5"

} >> $INSLATT_DIR/cassandra/conf/.jmxremote.password
